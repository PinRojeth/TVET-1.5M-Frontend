...
@Component({
  selector: 'app-report-enrollment-major',
  templateUrl: './report-enrollment-major.component.html',
  styleUrls: ['./report-enrollment-major.component.scss']
})
export class ReportEnrollmentMajorComponent implements OnInit {
  ...
  requestUrl: string = environment.api_url + this.reportService.path + '/student_apply_by_major';
  ...
  onLoad(): void {
    ...
    this.reportService
      .getApplyCountByMajor({ ...this.filterParams, start_date: startDate, end_date: endDate })
      .pipe(
        map(map => {
  ...
  trackByFn(index: number, item: any): void {
    ...
    const title = `ចំនួនចុះឈ្មោះតាមជំនាញចាប់ពី ${startDate} ដល់ ${endDate}`;
