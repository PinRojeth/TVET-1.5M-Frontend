...
@Component({
  ...
  templateUrl: './report-enrollment.component.html',
  styleUrls: ['./report-enrollment.component.scss']
})
export class ReportEnrollmentComponent implements OnInit {
        ...
        map(map => {
          if (map?.report_data?.length > 0) {
            ...
            for (const body of map.report_data) {
              ...
              //to sum up all total and female values
              // let total = 0;
              // let female = 0;
              // let totalHeader = body.schools.map(map => map?.student_data[map?.student_data?.length - 1]);
              // total = totalHeader.reduce((accumulator, currentValue) => {
              //   return accumulator + currentValue.total_student;
              // }, 0);
              // female = totalHeader.reduce((accumulator, currentValue) => {
              //   return accumulator + currentValue.total_female;
              // }, 0);
            }
  ...
  onExportFile(): void {
    ...
    const title = `ចំនួនចុះឈ្មោះតាមគ្រឹះស្ថានចាប់ពី ${startDate} ដល់ ${endDate}`;
