...
@Component({
  selector: 'app-report-approval-by-all',
  templateUrl: './report-approval-by-all.component.html',
  styleUrls: ['./report-approval-by-all.component.scss']
})
export class ReportApprovalByAllComponent {
  ...
  onLoad(): void {
    ...
    this.reportService
      .getApprovalStudentCityProvince({ ...this.filterParams, start_date: startDate, end_date: endDate })
      .pipe(
        map(map => {
  ...
  onExportFile(): void {
    ...
    const title = `ចំនួនអនុម័តតាមរាជធានី-ខេត្ត, គ្រឹះស្ថាន អ.ប.វ., វិស័យ និងមុខជំនាញ ចាប់ពី ${startDate} ដល់ ${endDate}`;
